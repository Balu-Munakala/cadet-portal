...
exports.createAchievement = async (req, res) => {
  ...
  try {
    // 1) Insert into achievements using PostgreSQL syntax
    ...
    const newAchievementId = achievementResult.rows[0].achievement_id;
    ...
    const insertPromises = cadets.map((c) =>
      pool.query(
        `INSERT INTO notifications (regimental_number, type, message, link)
         VALUES ($1, $2, $3, $4)`,
        [c.regimental_number, 'Achievement', message, link]
      )
    );
  ...
  try {
    ...
    const checkResult = await pool.query(
      `SELECT title, image_path
         FROM achievements
       WHERE achievement_id = $1 AND ano_id = $2`,
